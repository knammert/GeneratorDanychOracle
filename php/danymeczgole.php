<?php 
        ...
        if(($row= oci_fetch_row($query))==false){ 
         ...
         //MAXID dla DANYMECZGOLE
         $sql = "SELECT IDGOL,to_char(DATA, 'dd-mon-yy hh24:mi:ss' ) FROM DANYMECZGOLE WHERE IDGOL  = (SELECT MAX(IDGOL) FROM DANYMECZGOLE)";
         $query= oci_parse($c, $sql);
         oci_execute($query);
         if(($row= oci_fetch_row($query))!=false){  
                 $MAXID=$row[0];
         ...
         //dodawanie rekrdów
        for($i=0 ; $i<$danymeczgoleCounter; $i++){
            $MAXID++;
            //Zmiana daty
        ...
        $sql ="INSERT INTO DANYMECZGOLE(IDGOL,DATA,MECZ_IDMECZ,ZAWODNIK_IDZAWODNIK) VALUES
         (:IDGOL,to_date(:DATA,'yyyy/mm/dd hh24:mi:ss'),:MECZ_IDMECZ,:ZAWODNIK_IDZAWODNIK)";
        $query= oci_parse($c, $sql);    
        oci_bind_by_name($query, ':IDGOL',$MAXID);
        ...
        $tablicInsertow[]= "INSERT INTO DANYMECZGOLE(IDGOL,DATA,MECZ_IDMECZ,ZAWODNIK_IDZAWODNIK) VALUES (".$MAXID.",'".$DATA."','".$MECZ_IDMECZ."',".$ZAWODNIK_IDZAWODNIK.");";
         }
